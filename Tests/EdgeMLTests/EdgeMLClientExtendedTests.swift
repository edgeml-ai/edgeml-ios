...
/// Extended tests for ``EdgeMLClient`` covering guard clauses in extension
/// methods that require device registration.
///
/// Every method that checks ``self.deviceId`` should throw
/// ``EdgeMLError.deviceNotRegistered`` when called before ``register()``.
final class EdgeMLClientExtendedTests: XCTestCase {

    private static let testHost = "api.test.edgeml.ai"
    private static let testServerURL = URL(string: "https://\(testHost)")!

    private func makeClient() -> EdgeMLClient {
        return EdgeMLClient(
    ...
    func testDefaultServerURL() {
        XCTAssertEqual(EdgeMLClient.defaultServerURL.scheme, "https")
        XCTAssertEqual(EdgeMLClient.defaultServerURL.host, "api.edgeml.ai")
    }
}
