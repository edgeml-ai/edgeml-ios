...
final class PairingViewModelTests: XCTestCase {
    ...
    func testInitialStateIsConnecting() {
        let vm = PairingViewModel(token: "TEST123", host: "https://api.edgeml.io")

        if case .connecting(let host) = vm.state {
            XCTAssertEqual(host, "https://api.edgeml.io")
        } else {
    ...
    func testRetryResetsToConnecting() async {
        let vm = PairingViewModel(token: "TEST", host: "https://api.edgeml.io")
    ...
    func testHostWithoutSchemeGetsHttpsPrefix() async {
        ...
        let vm = PairingViewModel(token: "TEST", host: "api.edgeml.io")

        // The initial state should still capture the raw host
        if case .connecting(let host) = vm.state {
            XCTAssertEqual(host, "api.edgeml.io")
        } else {
            XCTFail("Expected connecting state")
        }
    }
...
/// Tests for the URL parsing logic used by ``EdgeMLPairingModifier``.
...
final class PairingDeepLinkParsingTests: XCTestCase {
    ...
    func testCustomSchemeDeepLink() {
        let result = parseDeepLink("edgeml://pair?token=ABC123&host=https://api.edgeml.io")
        XCTAssertNotNil(result)
        XCTAssertEqual(result?.token, "ABC123")
        XCTAssertEqual(result?.host, "https://api.edgeml.io")
    }

    func testCustomSchemeWithoutHost() {
        let result = parseDeepLink("edgeml://pair?token=XYZ")
        XCTAssertNotNil(result)
        XCTAssertEqual(result?.token, "XYZ")
        XCTAssertEqual(result?.host, "https://api.edgeml.io") // default
    }

    func testHttpsUniversalLink() {
        let result = parseDeepLink("https://app.edgeml.io/pair?token=TEST&host=https://custom.server.io")
        XCTAssertNotNil(result)
    ...
    func testMissingTokenReturnsNil() {
        let result = parseDeepLink("edgeml://pair?host=https://api.edgeml.io")
        XCTAssertNil(result)
    }

    func testEmptyTokenReturnsNil() {
        let result = parseDeepLink("edgeml://pair?token=&host=https://api.edgeml.io")
        XCTAssertNil(result)
    }

    func testNonPairSchemeReturnsNil() {
        let result = parseDeepLink("edgeml://settings?token=ABC123")
        XCTAssertNil(result)
    }
    ...
    func testCustomHostParameter() {
        let result = parseDeepLink("edgeml://pair?token=CODE42&host=http://192.168.1.100:8000")
        XCTAssertNotNil(result)
