...
///
/// Because ``OctomilModel`` requires a real ``MLModel`` (which needs a compiled
/// .mlmodel fixture), cache-hit tests verify the protocol delegation path
/// rather than constructing full model objects.
final class ModelManagerTests: XCTestCase {

    private static let testServerURL = URL(string: "https://test.octomil.com")!

    private var mockCache: MockModelCache!
    private var config: OctomilConfiguration!
    ...
    func testGetLatestDelegatesToCache() {
        ...
        let manager = makeManager()
        // The mock returns nil (can't create real OctomilModel), but verifies delegation
    ...
    func testClearCacheThrowsWhenCacheThrows() async {
        ...
        do {
            ...
            if case OctomilError.cacheError = error {
                // Expected
            } else {
    ...
    func testConcurrentDownloadsSameModelDeduplicateRequests() async {
        ...
        async let result1: Result<OctomilModel, Error> = {
            do {
        ...
        async let result2: Result<OctomilModel, Error> = {
            do {
                let model = try await manager.downloadModel(modelId: "concurrent-model", version: "1.0")
                return .success(model)
            } catch {
...
/// Returns nil for model lookups (can't construct real OctomilModel without .mlmodel)
...
private final class MockModelCache: ModelCaching, @unchecked Sendable {
    ...
    func clearAll() throws {
        if clearShouldThrow {
            throw OctomilError.cacheError(reason: "Mock clear error")
        }
        clearAllCalled = true
        size = 0
    }
