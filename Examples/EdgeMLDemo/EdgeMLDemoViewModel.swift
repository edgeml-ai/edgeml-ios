...
/// View model for the EdgeML Demo app
@MainActor
class EdgeMLDemoViewModel: ObservableObject {

    // MARK: - Published Properties

    @Published var isRegistered = false
    @Published var deviceId: String?
    @Published var model: EdgeMLModel?
    ...
    private var client: EdgeMLClient?
    ...
    private let serverURL: URL = {
        let host = ProcessInfo.processInfo.environment["EDGEML_SERVER_HOST"] ?? defaultHost
        let port = ProcessInfo.processInfo.environment["EDGEML_SERVER_PORT"].flatMap(Int.init) ?? defaultPort
        guard let url = URL(string: "http://\(host):\(port)") else {
            fatalError("Invalid EDGEML_SERVER_HOST or EDGEML_SERVER_PORT")
        }
        return url
    }()
    ...
    private func setupClient() {
        let configuration = EdgeMLConfiguration.development
        client = EdgeMLClient(
            apiKey: apiKey,
            orgId: orgId,
            serverURL: serverURL,
    ...
    func runTraining() async {
        ...
        guard model.supportsTraining else {
            handleError(EdgeMLError.trainingNotSupported)
            return
        }
        ...
        do {
            ...
            if batch.count == 0 {
                handleError(EdgeMLError.trainingFailed(reason: "Training batch is empty"))
                return
            }
    ...
    private func handleError(_ error: Error) {
        if let edgeMLError = error as? EdgeMLError {
            errorMessage = edgeMLError.localizedDescription
        } else {
    ...
    private func createSampleInput(for model: EdgeMLModel) -> MLFeatureProvider {
