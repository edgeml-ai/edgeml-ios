...
public final class ModelCache: ModelCaching, @unchecked Sendable {
    ...
    private var memoryCache: [String: EdgeMLModel] = [:]
    ...
    internal init(maxSize: UInt64) {
        ...
        self.logger = Logger(subsystem: "ai.edgeml.sdk", category: "ModelCache")

        // Get cache directory
        let cacheDir = fileManager.urls(for: .cachesDirectory, in: .userDomainMask).first!
        self.cacheDirectory = cacheDir.appendingPathComponent("ai.edgeml.models", isDirectory: true)
    ...
    internal func store(_ model: EdgeMLModel) {
    ...
    internal func get(modelId: String, version: String) -> EdgeMLModel? {
    ...
    internal func getLatest(modelId: String) -> EdgeMLModel? {
        lock.lock()
    ...
    private func loadModelFromDisk(modelId: String, version: String, path: URL) -> EdgeMLModel? {
        do {
