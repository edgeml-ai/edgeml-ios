...
/// Configuration for an ``EdgeMLWrappedModel``.
///
/// Controls validation, telemetry collection, and OTA update behaviour
/// when wrapping a CoreML ``MLModel`` with ``EdgeML/wrap(_:modelId:config:)``.
///
/// ```swift
/// var config = EdgeMLWrapperConfig.default
/// config.telemetryEnabled = false  // disable for local-only usage
/// let model = try EdgeML.wrap(coreModel, modelId: "classifier", config: config)
/// ```
public struct EdgeMLWrapperConfig: Sendable {

    /// Shared default configuration.
    public static let `default` = EdgeMLWrapperConfig()
    ...
    /// Base URL of the EdgeML server.  Required for telemetry and OTA.
