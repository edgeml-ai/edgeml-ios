...
//  EdgeML iOS SDK
//
//  Advertises an EdgeML device on the local network via Bonjour (mDNS)
//  so that the CLI (`edgeml deploy --phone`) can discover it.
//
...
/// so the EdgeML CLI can discover it for model deployment.
///
/// Uses `NWListener` from Network.framework (not the deprecated `NetService`).
///
/// The listener advertises an `_edgeml._tcp` service with TXT record metadata
...
/// // Device is now discoverable by `edgeml deploy --phone`
...
public final class BonjourAdvertiser: @unchecked Sendable {
    ...
    /// Bonjour service type for EdgeML device discovery.
    public static let serviceType = "_edgeml._tcp"
    ...
    public init(deviceId: String, deviceName: String? = nil) {
        ...
        self.logger = Logger(subsystem: "ai.edgeml.sdk", category: "BonjourAdvertiser")
    }
    ...
    public func startAdvertising(port: UInt16 = 0) throws {
        ...
        let queue = DispatchQueue(label: "ai.edgeml.bonjour")
