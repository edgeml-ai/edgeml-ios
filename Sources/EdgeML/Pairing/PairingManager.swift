...
///
/// The pairing flow is:
/// 1. User scans a QR code on the EdgeML dashboard
...
///
/// ```swift
/// let manager = PairingManager(serverURL: URL(string: "https://api.edgeml.ai")!)
...
public actor PairingManager {
    ...
    public init(apiClient: APIClient, configuration: EdgeMLConfiguration = .standard) {
        self.apiClient = apiClient
        self.configuration = configuration
        self.logger = Logger(subsystem: "ai.edgeml.sdk", category: "PairingManager")
    }
    ...
    ///   - configuration: SDK configuration.
    public init(serverURL: URL, configuration: EdgeMLConfiguration = .standard) {
    ...
    /// - Throws: ``PairingError`` or ``EdgeMLError``.
    public func connect(
        code: String,
