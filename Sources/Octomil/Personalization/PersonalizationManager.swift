...
public actor PersonalizationManager {
    ...
    private var personalizedModel: OctomilModel?
    private var baseModel: OctomilModel?
    ...
    public init(
        ...
        self.logger = Logger(subsystem: "ai.octomil.sdk", category: "PersonalizationManager")
    }
    ...
    public func setBaseModel(_ model: OctomilModel) {
        ...
        if let personalizedURL = getPersonalizedModelURL(for: model.id),
           FileManager.default.fileExists(atPath: personalizedURL.path) {
            do {
                ...
                self.personalizedModel = OctomilModel(
                    id: model.id,
    ...
    public func getCurrentModel() -> OctomilModel? {
    ...
    public func resetPersonalization() throws {
        guard let model = baseModel else {
            throw OctomilError.trainingFailed(reason: "No base model loaded")
        }
    ...
    public func trainIncrementally() async throws {
        ...
        guard let model = getCurrentModel() else {
            throw OctomilError.trainingFailed(reason: "No model available for training")
        }
    ...
    public func forceTraining() async throws {
        guard !trainingBuffer.isEmpty else {
            throw OctomilError.trainingFailed(reason: "No training samples in buffer")
        }
    ...
    private func getPersonalizedModelURL(for modelId: String) -> URL? {
        ...
        let modelsDirectory = documentsURL.appendingPathComponent("Octomil/PersonalizedModels")
    ...
    private func savePersonalizedModel() async throws {
        ...
        guard let personalizedURL = getPersonalizedModelURL(for: model.id) else {
            throw OctomilError.trainingFailed(reason: "Could not determine personalized model URL")
        }
        ...
        let mlModel = try MLModel(contentsOf: personalizedURL)
        personalizedModel = OctomilModel(
            id: model.id,
            version: "\(model.version)-personalized",
            mlModel: mlModel,
